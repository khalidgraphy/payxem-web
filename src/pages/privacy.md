---
layout: legal.njk
title: "Privacy Policy - Payxem"
description: "Payxem Privacy Policy - Learn how we collect, use, store, and protect your personal information."
lastUpdated: "24 April 2026"
intro: "Payxem (\"Payxem\", \"we\", \"our\", or \"us\") respects your privacy and is committed to protecting your personal information. This Privacy Policy explains how we collect, use, store, and share your information when you use our website and services.<br><br>By using Payxem, you agree to the practices described in this policy."
permalink: /privacy/
isHome: false
robots: "index, follow"
---

## 1. What Payxem Does

Payxem is the Digitalancers Platform, a technology platform for three connected products: **Payments** (invoices, pay links, and a ledger wallet at app.payxem.com), **Digital Business Card** (pxem.link/@username), and **Portfolio** ({username}.payxem.com). We enable users to create invoices, send payment links, track payments, manage earnings, and publish public profile pages.

Payments are processed through third-party providers such as PayPal and Stripe. Payxem does not operate as a bank or licensed financial institution.

## 2. Information We Collect

We collect information to provide and improve our services.

### a. Information You Provide

- Name and profile details
- Email address
- Phone number (optional)
- Business or service information
- Bank or payout details (when provided)
- Identity verification information (for KYC, when required)

### b. Transaction and Usage Data

- Invoice details (amounts, descriptions, client data)
- Payment records and statuses
- Wallet and ledger activity
- Withdrawal and payout requests

### b2. Public Profile Content (Digital Card and Portfolio)

When you publish a Digital Business Card or Portfolio page, we store and display:

- Your username, display name, avatar, verified (KYC) status, and bio or tagline
- Contact methods you choose to show (WhatsApp, email, phone, socials)
- Portfolio content you create (about sections, services, testimonials, FAQ, gallery images, hero images)
- Quote requests submitted to your Portfolio by visitors (name, email, message)

This content is intentionally public and is served at pxem.link/@username and {username}.payxem.com. You control what is published and can unpublish at any time from your dashboard.

### b3. Public Page Visitor Logs

For visits to your Digital Business Card or Portfolio, we may log basic request-level data (IP address, approximate country, user agent, timestamp, referrer) to prevent abuse, rate-limit traffic, and understand aggregate usage of the platform.

Payxem does not currently run an in-app QR scanner. When someone scans your Card or Portfolio QR with their phone's native camera, it simply opens the public page in their browser, and the visit is logged the same way as any other web visit.

### c. Device and Technical Data

- IP address
- Browser type and device information
- Usage logs and activity within the platform

### d. Cookies and Tracking

We use cookies and similar technologies to:

- Keep you logged in
- Improve performance
- Understand usage patterns

## 3. How We Use Your Information

We use your data to:

- Provide and operate Payxem services
- Create and manage invoices and payment records
- Process withdrawals and payouts
- Verify identity (KYC where required)
- Improve platform performance and security
- Communicate important updates and support responses
- Prevent fraud, abuse, and unauthorized activity

## 4. Payment Processing

All payments are handled by third-party providers such as PayPal and Stripe.

- Payment data may be processed directly by these providers
- Payxem may receive payment status and transaction details
- Funds may be received into accounts designated by Payxem or its affiliated entities for settlement and distribution

Payxem does not provide banking or custodial financial services.

## 5. Sharing of Information

We do not sell your personal data.

We may share information with:

### a. Payment Providers

To process transactions and verify payments.

### b. Service Providers

Hosting, analytics, email delivery, and infrastructure services.

### c. Identity Verification Partners

For KYC and compliance checks (when required).

### d. Legal Requirements

If required by law, regulation, or to protect rights and safety.

## 6. Data Storage and Security

We implement industry-standard measures to protect your data, including:

- Encrypted connections (HTTPS)
- Secure authentication and session management
- Access controls and monitoring
- Regular system updates

However, no system is completely secure. You are responsible for keeping your login credentials safe.

## 7. Data Retention

We retain your data as long as necessary to:

- Provide services
- Maintain financial and transaction records
- Comply with legal obligations

You may request deletion of your account, subject to legal and operational requirements.

## 8. Your Rights

Depending on your location, you may have the right to:

- Access your personal data
- Update or correct information
- Request deletion of your data
- Object to certain data uses

To make a request, contact us at: [support@payxem.com](mailto:support@payxem.com)

## 9. Third-Party Links and Services

Payxem may include links or integrations with third-party services.

We are not responsible for the privacy practices of these third parties. You should review their policies separately.

## 10. International Use

Payxem is designed for global use, including regions where traditional payment systems may be limited.

By using Payxem, you acknowledge that your data may be processed in multiple countries where our services or partners operate.

## 11. Updates to This Policy

We may update this Privacy Policy from time to time.

Changes will be posted on this page with an updated "Last updated" date. Continued use of the platform means you accept the updated policy.

## 12. Contact Us

If you have questions or concerns about this Privacy Policy, contact us at:

Email: [support@payxem.com](mailto:support@payxem.com)
